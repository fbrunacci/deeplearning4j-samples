...
object IrisMergeVertexSample {
    ...
    fun main(args: Array<String>) {
        ...
        val petalAndSepalMatrix = Array(row) { DoubleArray(4) }
        val petalMatrix = Array(row) { DoubleArray(2) }
        val sepalMatrix = Array(row) { DoubleArray(2) }
        for (r in 0 until row) {
            for (c in 0 until 4) {
                petalAndSepalMatrix[r][c] = irisWithoutLabel[c][r] as Double
            }
        }
        for (r in 0 until row) {
            for (c in 0 until 2) {
        ...
        println(Arrays.deepToString(petalMatrix.take(5).toTypedArray()).replace("], ", "]\n")+"\n...")
        println("sepal length/width")
        println(Arrays.deepToString(sepalMatrix.take(5).toTypedArray()).replace("], ", "]\n")+"\n...")
        println("full data length/width")
        println(Arrays.deepToString(petalAndSepalMatrix.take(5).toTypedArray()).replace("], ", "]\n")+"\n...")
...
//Convert the data matrices into training INDArrays
        val dataPetalIn = Nd4j.create(petalMatrix)
        val dataSepalIn = Nd4j.create(sepalMatrix)
        val dataPetalAndSepalIn = Nd4j.create(petalAndSepalMatrix)
        val dataOut = Nd4j.create(twodimLabel)

        val seed: Long = 6
        val petalOnlyEval = runNet("Petal", seed, dataPetalIn, twodimLabel)
        val sepalOnlyEval = runNet("Sepal", seed, dataSepalIn, twodimLabel)

        val merged_conf = NeuralNetConfiguration.Builder()
        ...
        val mergedModel = ComputationGraph(merged_conf)
        mergedModel.init()
        ...
        val trainingDataSet = MultiDataSet(arrayOf(trainingPetalDataSet,trainingPetalDataSet),arrayOf(trainingData.labels))
    ...
    private fun runNet(dataName: String, seed: Long, dataIn: INDArray, twodimLabel: Array<DoubleArray>): Evaluation {
        val dataOut = Nd4j.create(twodimLabel)
